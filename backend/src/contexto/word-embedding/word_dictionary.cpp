...
bool WordDictionary::LoadFromVectorFile(std::string_view file_path, size_t max_words) {
  ...
  if (vector_size != models::kEmbeddingDimension) {
    LOG_ERROR() << "Unexpected embedding dimension: " << vector_size << ", expected: " << models::kEmbeddingDimension;
    return false;
  }

  // Limit the number of words if specified
  if (max_words > 0 && max_words < vocabulary_size) {
    vocabulary_size = max_words;
  }

  LOG_INFO() << "Loading " << vocabulary_size << " word embeddings with dimension " << vector_size;
  ...
  type_index_.clear();
  words_.reserve(vocabulary_size);
  word_index_.reserve(vocabulary_size);
  normalized_word_index_.reserve(vocabulary_size);

  size_t loaded_words = 0;
  while (std::getline(file, line) && loaded_words < vocabulary_size) {
    std::istringstream iss(line);
    std::string word_with_pos;
    if (!(iss >> word_with_pos)) continue;  // Skip invalid lines

    // Parse word and POS tag
    std::string word = word_with_pos;
    ...
    if (pos_separator != std::string::npos) {
      ...
      word = std::string(word_view);  // Only create string when needed
    ...
    word_index_[words_[index].word] = index;
    normalized_word_index_[words_[index].normalized_word] = index;
    type_index_[words_[index].type].push_back(index);

    ++loaded_words;

    if (loaded_words % 10000 == 0) {
      LOG_INFO() << "Loaded " << loaded_words << " word embeddings";
    }
  }

  LOG_INFO() << "Successfully loaded " << words_.size() << " word embeddings";
  return !words_.empty();
}
...
const models::DictionaryWord* WordDictionary::FindWord(std::string_view word) const {
  ...
  {
    auto it = word_index_.find(std::string(word));
  ...
  auto it = normalized_word_index_.find(normalized);
  if (it != normalized_word_index_.end()) {
    return &words_[it->second];
  }
...
double WordDictionary::CalculateSimilarity(std::string_view word1, std::string_view word2) const {
...
std::vector<std::pair<const models::DictionaryWord*, double>> WordDictionary::GetMostSimilarWords(std::string_view word,
                                                                                                  size_t count) const {
  const models::DictionaryWord* dict_word = FindWord(word);
  if (!dict_word) return {};  // Word not found

  // Calculate similarity with all words
  using WordWithSimilarity = std::pair<const models::DictionaryWord*, double>;
  ...
  for (const auto& other_word : words_) {
    ...
    const double sim = dict_word->CalculateSimilarity(other_word);
    similarities.emplace_back(&other_word, sim);
  }

  // Sort by similarity in descending order
  std::partial_sort(similarities.begin(), similarities.begin() + std::min(count, similarities.size()),
                    similarities.end(),
                    [](const WordWithSimilarity& a, const WordWithSimilarity& b) { return a.second > b.second; });
