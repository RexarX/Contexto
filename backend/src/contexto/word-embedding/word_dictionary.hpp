...
public:
  ...
  double CalculateSimilarity(std::string_view word1, std::string_view word2) const;

  bool ContainsWord(std::string_view word) const { return FindWord(word) != nullptr; }

  std::vector<std::pair<const models::DictionaryWord*, double>> GetMostSimilarWords(std::string_view word,
...
private:
  ...
  std::unordered_map<std::string, size_t> word_index_;             // Original word -> index
  std::unordered_map<std::string, size_t> normalized_word_index_;  // Normalized word -> index
