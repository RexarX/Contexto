...
# Default values
USERVER_PATH="https://raw.githubusercontent.com/userver-framework/userver/refs/heads/develop/scripts/docs/en/deps"
...
install_arch() {
    local deps_url="$USERVER_PATH/arch.md"
    
    print_info "Installing dependencies for Arch-based system ($OS)..."
    print_info "Fetching dependencies from $deps_url"

    # Fetch the dependencies file
    local deps_content
    if ! deps_content=$(wget -q -O - "$deps_url"); then
        print_error "Failed to fetch dependencies from $deps_url"
        exit 1
    fi
